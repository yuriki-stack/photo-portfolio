...
function setText(el,ja,en){if(el)el.textContent=lang()==='en'?en:ja}
...
function renderFeatured(){
  ...
  fg.innerHTML=top.length?top.map(p=>cardHTML(p,-1,true)).join(''):`<p class="section-note" data-ja="写真を追加するとここに表示されます。" data-en="Your featured photos will appear here.">写真を追加するとここに表示されます。</p>`;
...
function cardHTML(p,i,feature=false){
  ...
  return `<article class="card" data-${feature?'feature':'i'}="${esc(feature?p.image:i)}">
    <div class="photo-wrap">
      ...
      <button class="favorite-badge ${fav?'is-favorite':''}" data-favorite="${feature?'feature:'+esc(p.image):i}" aria-label="Favorite">${fav?'★':'☆'}</button>
      <button class="like-badge" data-like="${feature?'feature:'+esc(p.image):i}" aria-label="Like">♡ ${liked}</button>
    </div>
...
async function init(){
  try{
    const r=await fetch('photos.json?'+Date.now());
    if(!r.ok)throw new Error('photos.json');
    photos=await r.json();
    buildFilters();buildDateFilters();apply();renderFeatured();
    await loadGlobalLikes();
  }catch(e){grid.innerHTML='<p>写真データを読み込めませんでした。</p>';}
}
function buildFilters(){
  const cats=['All',...new Set(photos.map(p=>p.category).filter(Boolean))];
  filters.innerHTML=cats.map(c=>`<button class="${c===selectedCategory?'active':''}" data-cat="${esc(c)}">${esc(categoryLabel(c))}</button>`).join('');
...
function buildTagFilters(){
  ...
  tagFilters.innerHTML=tags.map(t=>`<button class="${t===selectedTag?'active':''}" data-tag="${esc(t)}">${t==='All'?(lang()==='en'?'All tags':'すべてのタグ'):'#'+esc(t)}</button>`).join('');
  tagFilters.onclick=e=>{if(!e.target.dataset.tag)return;selectedTag=e.target.dataset.tag;apply()};
}
...
function apply(){
  ...
  filtered=photos.filter(p=>{
    const date=p.date||'';
    const hay=[p.title,p.category,p.date,p.location,p.note,...(p.tags||[])].join(' ').toLowerCase();
...
$('#favoriteBtn').onclick=()=>{const p=filtered[current];if(!p)return;toggleFavorite(p.image);updateLikeUI()};
...
$('#sortBtn').onclick=()=>{newest=!newest;$('#sortBtn').textContent=lang()==='en'?(newest?'Newest ↓':'Oldest ↑'):(newest?'新しい順 ↓':'古い順 ↑');apply()};
...
let sx=0;modal.addEventListener('touchstart',e=>sx=e.changedTouches[0].screenX,{passive:true});modal.addEventListener('touchend',e=>{let dx=e.changedTouches[0].screenX-sx;if(Math.abs(dx)>50)move(dx>0?-1:1)},{passive:true});
...
document.addEventListener('click',async e=>{
  ...
  if(fb){e.stopPropagation();const raw=fb.dataset.favorite;const image=raw.startsWith('feature:')?raw.slice(8):filtered[+raw]?.image;if(image)toggleFavorite(image);return}
...
async function addLike(image){
  ...
  try{
    const r=await fetch(`${SUPABASE_URL}/rest/v1/photo_likes?on_conflict=image`,{method:'POST',headers:{apikey:SUPABASE_ANON_KEY,Authorization:`Bearer ${SUPABASE_ANON_KEY}`,'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({image,likes:(likes[image]||0)+1})});
    if(!r.ok)throw new Error();const rows=await r.json();if(rows[0])likes[image]=Number(rows[0].likes)||0;
  }catch(e){likes[image]=(likes[image]||0)+1;globalLikesEnabled=false;saveLikes()}
}
...
$('#langBtn').onclick=()=>{
  ...
  const input=$('#searchInput');input.placeholder=en?input.dataset.placeholderEn:input.dataset.placeholderJa;input.setAttribute('aria-label',en?'Search photos':'写真を検索');
  $('#langBtn').textContent=en?'JP':'EN';
  $('#closeModal').setAttribute('aria-label',en?'Close':'閉じる');$('#prevBtn').setAttribute('aria-label',en?'Previous photo':'前の写真');$('#nextBtn').setAttribute('aria-label',en?'Next photo':'次の写真');
  buildFilters();buildDateFilters();apply();renderFeatured();
};
